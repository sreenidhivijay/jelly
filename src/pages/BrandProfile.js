...
const subscriptionHistory = [
  {
    ...
    details: {
      Reels: [
        { title: 'Spring Launch Teaser', creator: 'Aurora Blake', status: 'In Progress' },
      ...
      Posts: [
        { title: 'Pastel Palette Reveal', creator: 'Luna Sloane', status: 'Drafting' },
      ...
      Stories: [
        ...
        { title: 'Poll: Pink or Blue?', creator: 'Aurora Blake', status: 'Posted' },
  ...
  },
  {
    ...
    details: {
      Reels: [
        { title: 'Valentine\'s Day Campaign', creator: 'Rose Red', status: 'Posted' },
        { title: 'Winter Skincare Routine', creator: 'Glow Up', status: 'Posted' },
        { title: 'Cozy Home Tour', creator: 'Homebody', status: 'Posted' },
        { title: 'February Favorites', creator: 'Trend Setter', status: 'Posted' }
      ],
      Posts: [
        { title: 'Love is in the Air', creator: 'Rose Red', status: 'Posted' },
        { title: 'Galentine\'s Brunch', creator: 'Social Bee', status: 'Posted' },
        { title: 'Product Flatlay', creator: 'Aesthetic Lens', status: 'Posted' },
        { title: 'Quote of the Day', creator: 'Mindful', status: 'Posted' },
        { title: 'Outfit of the Night', creator: 'Fashion Fwd', status: 'Posted' },
      ...
      Stories: [
        { title: 'Flash Sale Countdown', creator: 'Aurora Blake', status: 'Posted' },
        { title: 'Behind the Scenes', creator: 'Aurora Blake', status: 'Posted' },
        { title: 'Q&A Session', creator: 'Glow Up', status: 'Posted' },
        { title: 'Poll: Chocolate or Flowers?', creator: 'Social Bee', status: 'Posted' },
        ...
        { title: 'Customer Repost', creator: 'Community Mgr', status: 'Posted' },
...
function BrandProfile() {
  ...
  const [expandedMonth, setExpandedMonth] = useState(subscriptionHistory[0]?.month);
      ...
      <header className="creator-profile-header brand-profile-header">
        ...
        <div className="stats-grid">
          <div>
            ...
            <span className="metric-value">{tier.name} ({tier.price}/mo)</span>
            <button 
              onClick={() => navigate('/signup/business/subscription-tiers', { state: { isManageMode: true } })}
              style={{ background: 'none', border: 'none', color: '#666', textDecoration: 'underline', fontSize: '11px', cursor: 'pointer', padding: 0, display: 'block', marginTop: '4px' }}
            >
              Manage Subscription
            </button>
          </div>
          ...
          <section className="timeline-section">
            <div className="timeline-container">
              {(() => {
                ...
                return Object.keys(historyByYear).sort((a, b) => b - a).map(year => (
                    ...
                    <div className="timeline-year-cards">
                      {historyByYear[year].map((period) => (
                        <div 
                          ...
                          onClick={() => setExpandedMonth(expandedMonth === period.month ? null : period.month)}
                          style={{ cursor: 'pointer' }}
                        >
                          <div className="month-header">
                            <div>
                              <h3>{period.month}</h3>
                              <span className={`month-status ${period.isActive ? 'current' : ''}`}>{period.status}</span>
                            </div>
                            {period.stats && (
                              <div className="month-summary-pills">
                                {period.stats.map((stat, i) => (
                                  <span key={i} className="summary-pill">
                                    <strong>{stat.used}/{stat.total}</strong> {stat.type}
                                  </span>
                                ))}
                                <span className="expand-icon">{expandedMonth === period.month ? '−' : '+'}</span>
                              </div>
                            )}
                          </div>
                          
                          {expandedMonth === period.month && period.details && (
                            <div className="month-details-wrapper" onClick={(e) => e.stopPropagation()}>
                                ...
                                {['Reels', 'Posts', 'Stories'].map((type) => (
                                  <div key={type} className="detail-column">
                                    ...
                                    <div className="detail-list">
                                      {(period.details[type] || []).filter(item => {
                                        if (filter === 'All') return true;
                                        const isCompleted = ['Posted', 'Completed', 'Delivered'].includes(item.status);
                                        return filter === 'Completed' ? isCompleted : !isCompleted;
                                      }).length > 0 ? (period.details[type] || []).filter(item => {
                                        if (filter === 'All') return true;
                                        const isCompleted = ['Posted', 'Completed', 'Delivered'].includes(item.status);
                                        return filter === 'Completed' ? isCompleted : !isCompleted;
                                      }).map((item, i) => (
                                      ...
                                      )) : <div className="empty-category">No {filter.toLowerCase()} content</div>}
                                    </div>
                                  </div>
                                ))}
                              </div>
