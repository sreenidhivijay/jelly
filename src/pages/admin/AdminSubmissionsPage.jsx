...
import { useAdminSubmissions, useReviewSubmission } from '../../hooks/useAdmin';
import './AdminPages.css';

const STATUS_FILTERS = ['all', 'pending', 'approved', 'rejected'];

function AdminSubmissionsPage() {
  const { data: submissions = [], isLoading: loading, error } = useAdminSubmissions();
  ...
  const handleReview = (submissionId, decision) => {
    reviewMutation.mutate(
      { submissionId, decision, notes: reviewNotes },
      {
        onSuccess: () => {
          setSelected(null);
          setReviewNotes('');
        },
        onError: (err) => alert(err.message || 'Review failed.'),
      },
    );
  };
